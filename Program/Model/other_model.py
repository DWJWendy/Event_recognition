...
class Contrast_Classifer(object):
    def __init__(self,  tain_x_path, train_y_path, test_x_path, test_y_path):
        self.svm_model = SVC()
    ...
    def convert_one_hot(self, item_data):
        target_words = datapreprocess.read_file(path="../Data/pre_process/target_represent.csv")["Split_content"]
        all_words = set()
    ...
    def run(self):
        ...
        train_x = self.unified_structure(self.train_x.copy()["Sentence_vec"].apply(lambda x: do_format(x)[0]))
        train_y = np.array(self.train_y.copy()["Label"])
        ...
        train_x = self.convert_one_hot(item_data=self.train_x.copy()["Split_content"])
        test_x = self.convert_one_hot(item_data=self.test_x.copy()["Split_content"])
        predict_y = self.do_fit(train_x, train_y, test_x, self.svm_model)
        F_value, Recall_value, Precision_value = self.metrics(y_true=test_y, y_predict=predict_y)
...
if __name__ == "__main__":
    ...
    contrast_model = Contrast_Classifer(train_x_path, train_y_path, test_x_path, test_y_path)
    contrast_model.run()
