...
class Trigger(object):
    def __init__(self, train_path, corpora_split_path, model_path, test_data_path):
        self.trigger_path = "../Data_2/trigger/"
        ...
        self.test_split_data = datapreprocess.read_file(test_data_path)
    ...
    @staticmethod
    def sort_words(items, event=True, top=500):
        """
        根据频数对其排序,选取前top个数据
        :param event:
        ...
        for item in items:
            if item not in word_frequency:
                ...
                word_frequency[item] += 1
        logger.info("事件：{e}, 总计种子数{seed_words_num}".format(e=event, seed_words_num=len(word_frequency)))
        if len(items) >= top:
    ...
    def seed_trigger(self, seed_words, top):
        """
        :return: 每一类事件触发对应的words,且根据频数对其排序,选取前top个数据
        """
        top_seed_trigger = {}
        for event, value in seed_words.items():
            top_seed_trigger[event] = self.sort_words(items=value, event=event, top=top)
    ...
    @staticmethod
    def compute_vec(item, model, weight=False):
        """
    ...
    def seed_vec(self, e_trigger, filename, weight=False):
        ...
        for e, value in e_trigger.items():
            seed_vec[e] = self.compute_vec(item=value, model=model, weight=weight)
    ...
    def add_new_word(self, threshold=0.8, top=2000, enrich=True, source_from=1, new=False):
        """
        :param threshold: 新词识别的相似度（default：0.8）
        :param top: 丰富动词来自前top个高频动词
        :param enrich: 是否丰富：True表示丰富,False表示直接用人工标注的种子数据
        :param source_from: 对丰富来源的选择：1=表示语料库,2=测试集
        :param new:
        :return:
        """
        if enrich:
            ...
            if source_from == 1:
                ...
                # 加载测试集中高频新词
                corpora_split_content = self.test_split_data.copy()
            model = Word2Vec.load(self.model_path)
                ...
                for item in high_frequency_words:
                    if item[0] not in new_trigger_words_list:
                        ...
                        if cosine_value and max(cosine_value) >= threshold:
                            ...
                            new_trigger_words[event].append((item[0], 1))
            ...
            datapreprocess.write_json_file(path=self.trigger_path + "event_words_add_new.json",
            ...
            datapreprocess.write_json_file(path=self.trigger_path + "final_event_words.json",
    ...
    def compute_event_vec(self, weight_2):
        logger.info("计算最终的事件向量..")
        event_trigger_words = self.event_words
        e_vec = self.seed_vec(event_trigger_words, filename="event_vec.csv", weight=weight_2)
        return e_vec

    def run(self, weight_1=True, weight_2=True, threshold=0.7, top_1=500, top_2=2000, enrich=True, source_from=2, new=False):
        logger.info("计算种子词...")
        self.seed_words = self.form_seed_words()
        logger.info("计算前top个高频种子词")
        self.top_seed_trigger = self.seed_trigger(self.seed_words, top=top_1)
        # 计算种子向量
        filename = "seed_vec_"+str(weight_1)+"_"+".csv"
        logger.info("计算计算种子向量,权重：{weight}".format(weight=weight_1))
        self.seed_event_vec = self.seed_vec(self.top_seed_trigger, filename, weight=weight_1)
        # 丰富种子词形成触发器
        self.event_words = self.add_new_word(threshold=threshold, top=top_2, enrich=enrich,
                                             source_from=source_from, new=new)
        # 计算触发器向量
        self.event_vec = self.compute_event_vec(weight_2=weight_2)
        return self.event_vec


class Trigger_Classifer(Trigger):
    def __init__(self, train_path, corpora_split_path, test_vec_path, test_label_path, model_path,
                 weight_1, weight_2, threshold, top_1, top_2, enrich, source_from, new):
        Trigger.__init__(self, train_path, corpora_split_path, model_path, test_vec_path)
        self.event_vec = Trigger.run(self, weight_1=weight_1, weight_2=weight_2, threshold=threshold,
                                     top_1=top_1, top_2=top_2, enrich=enrich, source_from=source_from, new=new)
    ...
    def compute_event_cosine(self, item_x, event_vec, shreshold=0.8):
        """
...
if __name__ == "__main__":
    ...
    trigger = Trigger(train_path=train_path, corpora_split_path=corpora_split_path,
                      model_path=model_path, test_data_path=test_data_path)
    weight_1, weight_2, threshold, top_1, top_2, enrich, source_from, new = True, True, 0.7, 700, 2000, True, 1, False
    classifier = Trigger_Classifer(train_path, corpora_split_path, test_data_path, test_label_path, model_path, weight_1,
